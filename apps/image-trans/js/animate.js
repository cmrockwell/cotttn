...
//Does a 2D animation by entering the distance, heading, increment, speed, (callback and boundary are optional)
//NOTE if you want to use a boundary and not a call back then enter false for the callback. If you want to use the callback
//enter true
Animate.prototype.animate2D = function(distance,heading,increment,speed,callback,boundary){
	
	//if callback is true then assign to callback property
	if (callback){
	...
	*/
	...
	if (!match){
		...
		we reset to the stopping point that is supposed to be at. */
			...
			//if callback is true then call callback method
			if(this.callback){
				callback(this.ele);
			}
